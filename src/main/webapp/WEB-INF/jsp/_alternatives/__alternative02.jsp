	...
	<script type="text/javascript">
		...
		var charts = [];
		  ...
		  function syncTooltip(container, p) {
			  ...
			  console.log('syncTooltip() map.size() is ' + map.size());
		 	  $.each( map.values(), function( index, val){
			        console.log(val.container.id);
			  }); 
...
//console.log(i+': '+charts[i].container.id);
...
//		              console.log(data.length);
		              for(var j=0; j<data.length; j++){
		                ...
		                //	  //console.log(moment.utc(data[j].x).format("YYYY-MM-DD HH:mm:ss"));
		                  if (data[j].x === p){
		                	  console.log(charts[i].container.id + ' :: selected point p is: ' + moment.utc(p).format("YYYY-MM-DD HH:mm:ss"))
